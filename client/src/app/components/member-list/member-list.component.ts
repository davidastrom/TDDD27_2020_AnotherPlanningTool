...
export class MemberListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searchUsers$ = this.getAllUsersGql.watch().valueChanges.pipe(map(res => res.data))
  }
  ...
  formatter = (member: {__typename?: 'User', _id: string, username: string}) => member.username;
  
  search = (text$: Observable<string>) => text$.pipe(
    debounceTime(200),
    distinctUntilChanged(),
    switchMap(term => this.searchUsers$.pipe(
      map(users => users.allUsers)).pipe(
        map(users => users.filter(user => new RegExp(term, 'i').test(user.username)).slice(0, 10))
        ).pipe()
    )
  )
}
